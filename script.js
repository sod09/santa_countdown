const xmasEndDate = new Date ("December 25, 2021 24:00:00").getTime();
...
const runFunction = setInterval(() => {
  ...
  const hours = Math.floor((timeRemainingBeforeXmas % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));

  const minutes = Math.floor((timeRemainingBeforeXmas % (1000 * 60  * 60 * 60)) / (1000 * 60));
  ...
  const minutesDiv = document.querySelector(".minutes").innerHTML = ` ${minutes} minutes`;

  const secondsDiv = document.querySelector(".seconds").innerHTML = `${seconds} seconds`;
